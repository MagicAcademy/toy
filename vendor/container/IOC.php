...
class IOC
{
     ...
     * @param     string                   $alias 对$concrete的一个映射名称
     * @param     Closure or string        $concrete 一个匿名函数或一个字符串类名
     */
    public function bind(string $alias, $concrete)
    {
        
        if (!array_key_exists($alias, $this->Binding)) {
            $this->Binding[$alias] = [
                                            'concrete' => $concrete,
                                            'isSingle' => false,
                                            'single' => null,
    ...
    public function make(string $alias)
    {
        ...
        if ($target['isSingle']) {
            if ($target['alreadySingle']) {
                ...
                $this->Binding[$alias]['alreadySingle'] = true;
                $this->Binding[$alias]['single'] = $single;
                return $single;
            }
        } else {
    ...
    private function dependParameters(array $parameters)
    {
        ...
        foreach ($parameters as $parameter) {
            ...
            if (is_null($depend)) {
                ...
                $alias = substr($name, $position + 1);

                $depends[] = self::make($alias);
            }
        }
        return $depends;
    }
     ...
     */
    private function buildClass(ReflectionClass $reflect)
    {
     ...
     */
    private function buildClosure(ReflectionFunction $reflect)
    {
     ...
     * @param     [string]                   $alias [description]
     */
    public function setSingle(string $alias,$concrete)
    {
        $this->Binding[$alias] = [
                                        'concrete' => $concrete,
                                        'isSingle' => true,
